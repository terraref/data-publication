...
These data are from the Theis-Clima weather station mounted on the field scanner. This sensor provides a suite of additional measurements, including Y frequency measurements of each parameter, and X resolution of the downwelling the solar spectra. The sensors and data collection frequency are described in the section on sensor data. We describe both the full resolution data provided as netcdf files in the sensor data product named "envlog_netcdf" and the 5 minute aggregated data provided here as json files that were accessed using the geostreams API as described in the TERRA REF tutorials. These time series are not continuous \@ref(fig:aggregated-weather-dates)
